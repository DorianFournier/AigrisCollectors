...
//                                 get_angle_between_two_points(ship_pos,
//                                 point), ATTACKER_SPEED));
// }
...
//   send_command(generate_command(
//       MOVE_CMD, ship.ship_ID, get_angle_between_two_points(ship_pos,
//       base_pos), ship_speed));
// }
...
//     planet_num++) { if (game_data->planets[planet_num].busy_ship_ID ==
//     ship_id) { planet_id = planet_num; break; } else { flag = true;
//       }
//     }
//     if (flag)
//       planet_id = get_nearest_planet_available(ship_id, game_data);
...
//     // if (game_data->planets[planet_id].planet_status == FREE ||
//     //     game_data->planets[planet_id].planet_status ==
//     COLLECTING_INCOMING) if (game_data->planets[planet_id].planet_status ==
//     FREE) {
//       go_to_planet(game_data->ships[ship_id],
//     game_data->planets[planet_id]); set_planet_collection_status(ship_id,
//     planet_id, COLLECTING_INCOMING, game_data); } else if
//     (game_data->planets[planet_id].planet_status == COLLECTING_INCOMING &&
//                game_data->planets[planet_id].busy_ship_ID == ship_id) {
//       go_to_planet(game_data->ships[ship_id],
//     game_data->planets[planet_id]); } else if
//     (game_data->planets[planet_id].planet_status == COLLECTING ||
//                game_data->ships[ship_id].broken == 1) {
//       go_to_base(game_data->ships[ship_id], game_data->base,
//     COLLECTOR_SPEED);
//     }
